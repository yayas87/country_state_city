...
/**
 * Plugin implementation of the 'country_state_city_ward_widget' widget.
 *
 * @FieldWidget(
 *   id = "country_state_city_ward_widget",
 *   label = @Translation("Country state city ward widget"),
 *   field_types = {
 *     "country_state_city_ward_type"
 *   }
 * )
 */
class CountryStateCityWardWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  public function getWards($city_id) {
    ...
    if ($city_id) {
      ...
      $query = $this->entityTypeManager->getStorage('wardlist')->getQuery()
      ...
      $wards = [];
      if (count($ids) == 1) {
        $result = $this->entityTypeManager->getStorage('wardlist')->load(key($ids));
        $wards[$result->id()] = $result->getName();
      }
      elseif (count($ids) > 1) {
        $results = $this->entityTypeManager->getStorage('wardlist')->loadMultiple($ids);
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $ward_id = $ward_id ?? $value['ward'] ?? NULL;
    ...
    if ($state_id) {
      $element['city'] = [
        ...
        '#ajax' => [
          ...
          'progress' => [
            'type' => 'throbber',
            'message' => $this->t('Searching Wards...'),
          ],
        ],
      ];
    }
    ...
    if ($city_id) {
      $element['ward'] = [
        ...
        '#options' => $this->getWards($city_id),
        '#empty_option' => $this->t('-- Select an option --'),
        '#required' => $this->fieldDefinition->isRequired(),
        '#title' => !empty($this->getFieldSetting('ward_lable')) ? $this->getFieldSetting('ward_lable') : $this->t('Ward'),
        '#validated' => TRUE,
        '#attributes' => [
          'class' => [
            'csc-ward-details',
